...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 8044B515CEF
	for <lists+linux-pwm@lfdr.de>; Sat, 30 Apr 2022 14:35:15 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1356254AbiD3Mif (ORCPT <rfc822;lists+linux-pwm@lfdr.de>);
        Sat, 30 Apr 2022 08:38:35 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:45648 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S239427AbiD3Mie (ORCPT
        <rfc822;linux-pwm@vger.kernel.org>); Sat, 30 Apr 2022 08:38:34 -0400
Received: from relay07.th.seeweb.it (relay07.th.seeweb.it [IPv6:2001:4b7a:2000:18::168])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 1BA5C25EF;
        Sat, 30 Apr 2022 05:35:05 -0700 (PDT)
Received: from SoMainline.org (94-209-165-62.cable.dynamic.v4.ziggo.nl [94.209.165.62])
        (using TLSv1.3 with cipher TLS_AES_256_GCM_SHA384 (256/256 bits)
         key-exchange ECDHE (P-256) server-signature RSA-PSS (2048 bits) server-digest SHA256)
        (No client certificate requested)
        by m-r2.th.seeweb.it (Postfix) with ESMTPSA id 148333F799;
        Sat, 30 Apr 2022 14:35:04 +0200 (CEST)
Date:   Sat, 30 Apr 2022 14:35:02 +0200
...
Message-ID: <20220430123502.7od55knvxkw3sv6s@SoMainline.org>
...
Apologies for replying twice to the same patch with disjoint review.
...
Having seen most of the patch revisions over time, I'm now happy to add
yet another:

Reviewed-by: Marijn Suijten <marijn.suijten@somainline.org>

and, if relevant on top of a review:
...
Note that this actually holds for 1500ms HIGH on the start and 1000ms
LOW on the end.  That should be easily fixable, and is explained in more
detail below in my review on the appropriate lines of code.

Because of the same issue, the simple "255 500 0 500" pattern above
actually takes 1000ms at the start and stop (every) step.
...
The underlined part of the pattern does not seem to be contained in the
pattern string.  It's high once for 1000ms, then high another time for
500ms (and 500ms in between every step), then repeats.  It does not
become high for 500ms twice nor is it a palindrome that plays in
reverse.
...
How about showcasing this by copying the (fixed) pattern above and
changing the stop duration?  Or is that too verbose?
...
This could perhaps also use a copy-pastable example, for testing :).
Something that "gradually" breathes in and out and holds at the end.

Other than that I have to say this documentation is really, _really_
well written.  It's packed full of information yet very concise to read.
...
Nit: It seems the members here and in struct lpg_channel are all
misaligned: most should have two tab characters, and no spaces like
@triled_base and @num_channels.
...
So, same here.  Should it be a single space, or enough tab characters to
align them all?
...
Nit: If we're going for v15 anyway, could you reword this comment to be
...
... otherwise the last entry is used [to determine "high pause"].?
...
I'm having some trouble getting this bit of code to accept patterns of
length 2 (or palindromes of length 3 with actual_len = 2):
pattern[1].delta_t here represents the "middle" delta_t yet is also used
as hi_pause in these cases and is hence allowed to deviate from the
usual validation.

In case of palindromes pattern[2] is expected to take just as long as
pattern[1] yet actually represents the same value as pattern[0], but
this is probably a niche-enough use-case to not worry about.

Otherwise the limit of BIT(9) makes it impossible to have simple len-2
patterns like "255 1000 0 1000" even though these are trivially possible
with pauses.  In reality one can compute delta_t = min(BIT(9) - 1,
min(pattern[i].delta_t)) here and use lo/hi_pause to compensate the
rest.
...
As mentioned above in the RST documentation (and IIRC we discussed this
in a previous review and/or IRC), I think delta_t from the rest of this
pattern should be _subtracted_ from these values (and error if wrapping
around).  After all the pattern will execute as normal without pausing,
and these pausing times seem to be _additive_ on top of the pattern.

This can be clearly seen when running the pattern from the examples:

    "255 1000 0 500 255 500 0 500"

Timing this shows that LED is high for 1500ms (500ms pattern + 1000ms
lo_pause).  Similarly for the final low case, the LED stays low for
1000ms (500ms pattern + 500ms hi_pause).

Obviously the documentation has to be updated to explain that these
pauses (rather, the durations for the first and last entry) must be
identical to or higher than the delta_t for the rest of the pattern, not
lower.
